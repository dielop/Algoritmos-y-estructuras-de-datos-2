...
b) Int -> (Int -> Int)
   add x y = x + y   (En este caso add esta currificada y toma los argumentos de a uno.)
...
solucion con lista por comprension:
unique :: [Int] -> [Int]
...
scalarproduc :: [Int] -> [Int] -> Int 
scalarproduc xs ys = sum [x*y | (x,y) <- zip xs ys ]
...
codes :: [Char]->[Int]
codes [] = []
codes (x:xs) = [ord x] ++ codes xs ¡esto no se puede usar, porque no hay que usar ord!

HECHA POR EL PROFE:
code c = orden c 1 "abcdefghijklmnopqrstuvwxyz"
orden c n [x] = n
orden c n (x:xs) =  if c==x then n else orden c (n+1) xs

codes [] = []
codes (x:xs) = code x : codes xs 
(la funcion map hace lo mismo pero no puedo usar prelude)
...
restos :: [Int]->Int->[Int]
restos [] num = [] -- modulo de [] num y va a dar una []. Num es el numero que divide la lista
...
longitudes :: [[Int]]->[Int]
longitudes [] = []
longitudes(xxs:xs) = [length xxs] ++ longitudes xs -- funcion predefinida
